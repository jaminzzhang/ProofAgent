...
**Business Flow Tool Proposal Scope Narrowing**:
The post-admission Business Flow Skill Pack effect that may filter, prioritize, or explain already-bound Tool Contracts inside Tool Proposal Scope without creating tools, enabling tools, expanding parameters, relaxing approval, or bypassing parameter-source constraints.
_Avoid_: Skill Pack tool grant, business-flow tool install, stage-local tool authority, parameter expansion

**Action Tool Proposal Eligibility**:
The stricter Effective Tool Proposal Scope admission condition for state-changing tools, requiring explicit Agent binding, caller-surface allowance, policy-precheck allowance, approval requirement, Binder-generated idempotency support, side-effect classification, and any product-specific transactional restrictions to pass before the planner may propose the tool.
...
The planner-visible parameter projection derived from a Tool Contract for one tool inside Effective Tool Proposal Scope, including only proposal-safe identity, purpose, risk and approval summary, semantic result summary, allowed and required parameter names, basic parameter types, bounded enum or range constraints, short parameter descriptions, and call-budget hints.
_Avoid_: Complete Tool Contract schema, MCP native schema, Tool Source connection detail, credential reference, result schema, result field list, raw policy rule
...
_Avoid_: Full Agent tool catalog, full MCP tool catalog, complete execution schema dump
