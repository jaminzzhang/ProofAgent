...
- Batch upload remains a Dashboard V1 capability with at most 50 files per batch. Its later API must reserve capacity for the full batch atomically before writing quarantine bytes, fail staging as one batch if reservation or persistence fails, and then let each successfully staged file proceed independently through asynchronous **Operator Knowledge Document Upload Validation**.
