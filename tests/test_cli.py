...
def test_demo_reports_unbound_knowledge_outcomes_and_writes_artifacts(monkeypatch) -> None:
    from proof_agent.delivery import cli

    monkeypatch.setattr(cli, "DEMO_AGENT_PATH", cli.DEMO_AGENT_PATH.resolve())
    with runner.isolated_filesystem():
        result = runner.invoke(app, ["demo"])
        assert Path("runs/latest/trace.jsonl").is_file()
        assert Path("runs/latest/governance_receipt.md").is_file()
    assert result.exit_code == 0
    assert result.output.splitlines() == [
        "Proof Agent demo",
        "supported: REFUSED_NO_EVIDENCE",
