...
def test_one_space_allows_multiple_agents_without_grant_or_budget_widening(
    ...
    release = (
        ...
        .publish(
            PublishKnowledgeReleaseCommand(
                knowledge_space_id="space-shared",
                knowledge_base_id="base-policy",
                knowledge_source_version_ids=(source_version.version.knowledge_source_version_id,),
            )
        )
        .release
    )
    ...
    lifecycle.deprecate(
        ...
        idempotency_key="deprecate-queryable-release",
    )
    assert access.authorize(client_id="agent-alpha", request=request) == alpha
    assert access.authorize(client_id="agent-beta", request=request) == beta
    access.register_client(client_id="agent-gamma", bearer_token="gamma-secret-token-44")
    with pytest.raises(KnowledgeAccessConflict):
        access.grant_release_query(
            client_grant_id="grant-gamma",
            client_id="agent-gamma",
            knowledge_base_release_id=release.knowledge_base_release_id,
            ...
            effective_access_scope_digest="sha256:" + "c" * 64,
        )
    ...
    lifecycle.retire(
        ...
        operator_id="knowledge-operator",
        idempotency_key="retire-deprecated-release",
    )
    assert access.authorize(client_id="agent-alpha", request=request) is None
    assert access.authorize(client_id="agent-beta", request=request) is None

    over_budget = request.model_copy(
        update={
            "execution_budget": request.execution_budget.model_copy(update={"max_candidates": 101})
        }
    )
