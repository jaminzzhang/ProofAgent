...
docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" exec -T \
  -e PROOF_AGENT_DRAFT_MEMORY_AGENT_ID="$EXACT_AGENT_ID" \
  -e PROOF_AGENT_DRAFT_MEMORY_DRAFT_ID="$EXACT_DRAFT_ID" \
  -e PROOF_AGENT_DRAFT_MEMORY_EXPECTED_REVISION="$EXPECTED_DRAFT_REVISION" \
  api python /opt/proof-agent-local/disable_draft_memory.py
