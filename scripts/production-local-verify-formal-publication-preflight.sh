...
docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" exec -T \
  -e PROOF_AGENT_PREFLIGHT_AGENT_ID="$EXACT_AGENT_ID" \
  -e PROOF_AGENT_PREFLIGHT_DRAFT_ID="$EXACT_DRAFT_ID" \
  -e PROOF_AGENT_PREFLIGHT_DRAFT_REVISION="$EXACT_DRAFT_REVISION" \
  api python /opt/proof-agent-local/verify_formal_publication_preflight.py
