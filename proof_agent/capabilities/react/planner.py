...
def _planner_control_prompt() -> str:
    return (
        ...
        "Check clarification dependencies against the CURRENT user request; never ask for data "
        "only needed in hypothetical future follow-ups. Pre-purchase product/suitability advice "
        "does not require ownership or policy documents for that product. Ask about needs, "
        "budget and overall coverage only to finish suitability; existing-policy operations "
        "retain their case-specific requirements. Do not reintroduce excluded policy questions. "
