...
def _intent_control_prompt() -> str:
    return (
        ...
        "each field's dependency on a CURRENT explicit subtask before calling it required: "
        "omit fields that are only for hypothetical future follow-ups. Pre-purchase suitability "
        "does not require the user to own this product or provide its policy documents. "
        "Distinguish overall existing coverage (remaining suitability context) from specific "
        "policy details for an explicitly requested existing-policy claim/surrender/status query. "
        "Do not merge unrelated case data with a genuinely required identity or permission field. "
        "Search for "
