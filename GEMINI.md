...
*   **Run Deterministic Demo:** `uv run --extra dev proof-agent demo` (Runs without API keys, generates traces in `runs/history/`).
...
1. **Authoritative Design:** Always refer to `docs/Proof Agent 技术设计方案.md` as the source of truth for architectural decisions and module boundaries before planning code changes.
...
5. **Dashboard Read-Only Principle:** The Dashboard API and UI exist strictly for **observability** of artifacts in `runs/history/` (`trace.jsonl`, `governance_receipt.md`). They must not create a secondary execution path that bypasses the core Harness CLI/Workflow.
