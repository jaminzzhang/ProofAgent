...
export function KnowledgeDetailPage() {
  ...
  async function uploadDocument(file: File | undefined) {
    ...
    try {
      ...
      const upload = await uploadKnowledgeDocument(sourceId, {
        filename: file.name,
        content_type: file.type || contentTypeForName(file.name),
        content_base64: contentBase64,
        actor: 'dashboard',
      })
      setStatus(`${upload.filename} is ${upload.state}.`)
  ...
  return (
    <div className="max-w-6xl space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-4">
        ...
        <Metric label="Published Snapshot" value={source.published_snapshot_id ?? '-'} />
      </section>
          ...
          <div className="mt-4 divide-y divide-[var(--border)] rounded-md border border-[var(--border)]">
            {publications.map((publication) => (
              <div key={publication.publication_id} className="grid gap-2 px-4 py-3 text-sm md:grid-cols-[1fr_auto_auto]">
                ...
                <span className="font-mono text-xs text-[var(--text-muted)]">{publication.snapshot_id}</span>
                <span className="text-xs text-[var(--text-muted)]">{publication.document_count} docs</span>
