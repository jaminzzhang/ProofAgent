"""Provision one dedicated Release Reference service-client identity."""
...
DEFAULT_REFERENCE_CLIENT_ID = "proof-agent-formal-publication-reference"
...
def provision_reference_client(
    registry: ClientRegistry,
    *,
    client_id: str,
    bearer_token: str,
) -> None:
    """Idempotently bind one trace-safe identity to one credential digest."""
...
def main(environment: Mapping[str, str] | None = None) -> None:
    ...
    client_id = values.get(
        "KSS_REFERENCE_CLIENT_ID",
        DEFAULT_REFERENCE_CLIENT_ID,
    ).strip()
    if not client_id:
        raise RuntimeError("KSS_REFERENCE_CLIENT_ID is required")
    bearer_token = secret_environment_value(
        values,
        "KSS_REFERENCE_CLIENT_BEARER_TOKEN",
    )
    if not bearer_token:
        raise RuntimeError("KSS_REFERENCE_CLIENT_BEARER_TOKEN is required")
    provision_reference_client(
        PostgresKnowledgeAccessControl.from_dsn(dsn),
        client_id=client_id,
        bearer_token=bearer_token,
    )
    print(f"KSS Reference client {client_id} is registered", flush=True)
