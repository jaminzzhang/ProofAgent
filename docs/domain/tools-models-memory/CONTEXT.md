...
The configuration capability boundary for reusable knowledge assets. `knowledge_source.view` gates Source, document metadata, review, operation, version, capability, and deletion-eligibility reads. `knowledge_source.edit` gates Source creation and edits, document upload, replacement, document archive, routing metadata, retry, cancellation, and workbook import. `knowledge_source.review` gates correction, approval, and rejection of business metadata reviews. `knowledge_source.publish` gates publication validation and Source publication. `knowledge_source.archive` gates Source Archive, Restore, and Knowledge Source Physical Deletion, while `audit.view` separately gates the Audit tab and audit records. Knowledge Source API command requests do not carry actor fields; the API resolves Operator Identity Context server-side and records the resolved operator. V1 records importer, reviewer, and publisher identities without requiring three different natural people, preserving an API boundary on which a deployment policy may later enforce separation of duties.
