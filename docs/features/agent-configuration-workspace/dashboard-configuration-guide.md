...
## Workflow 简化入口（2026-09-10）

日常只配置“理解用户诉求”“规划下一步”“生成答案”的 Prompt，点击“保存 Workflow”一次
保存所有节点修改。固定模板不再需要选择；桌面左侧紧凑导航保留全部节点、条件分支与返回规划的循环，右侧直接显示配置；窄屏默认收起流程，展开选中节点后自动收起。点击节点切换配置，系统节点只读，既有值会保留。
高级设置提供实际可用的额外上下文与预览；技术信息收纳版本、分支、原始配置和 YAML。
旧版本中展示的审查节点 Prompt 与空 Context 开关不再作为日常可编辑项。
未保存修改受节点标记、模块导航和刷新提示保护；需要放弃时使用页面已有的放弃按钮。
详细字段取舍、运行时证据及验证范围见 [Workflow 配置梳理](workflow-simplification.md)。
