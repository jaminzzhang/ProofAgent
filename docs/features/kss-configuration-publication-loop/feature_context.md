...
| MAIN-6 | 运行使用 | Run 解析 Active Agent Version | exact KSS Query → Candidate Evidence → ProofAgent Admission | 引用回答或受治理失败结果 | 无 fallback、引用可回放、Admission 权威正确 | P1 | 已实现主体路径。TDD-06A 通过固定 synthetic Candidate 独立验证 production-local compatibility Scorer composition；不调用 KSS 或 Draft-selected answer model，不替代真实 exact Candidate 验证 |
