...
| 测试负责人 | 待确认 |
| 发布负责人 | 待确认 |
| 当前状态 | `PARTIAL_VERIFICATION`：TDD-01A/01B 本地接线、TDD-02A 至 02G Preparation 核心、TDD-03A 至 03F Reference/lifecycle 核心、TDD-04A lifecycle/reference-summary 只读 BFF、TDD-04B Profile → Synchronization BFF、TDD-04C Base Draft save/exact read → Preparation start/status 同源 secret-free BFF、TDD-04D 可选 one-shot Preparation execution runtime、TDD-04E controlled Preparation publication BFF，以及 TDD-04F controlled Preparation cancellation BFF 已有本地证据；ProofAgent verifier、后台 reconciler、ProofAgent reference-first publication、真实上游、Preparation 常驻执行进程、过期与 lifecycle/Reference 命令 BFF、终端操作者到 KSS audit 的委托身份链、生产 artifact-retention adapter、物理删除、affected-reference 明细/通知、Dashboard 页面、自动调度、系统级唯一发布入口、生产 retention 配置和生产切换尚未完成 |
