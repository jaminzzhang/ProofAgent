...
| 测试负责人 | 待确认 |
| 发布负责人 | 待确认 |
| 当前状态 | `PARTIAL_VERIFICATION`：TDD-01A/01B 本地接线、TDD-02A 至 02G Preparation 核心、TDD-03A 至 03E Reference 注册/可信注销、Release deprecation/ordinary retirement/emergency revocation 已有本地证据；ProofAgent verifier、后台 reconciler、ProofAgent reference-first publication、真实上游、lifecycle 命令 HTTP/BFF、affected-reference 明细/通知、自动调度、系统级唯一发布入口、生产 retention 配置和生产切换尚未完成 |
...
flowchart LR
    ...
    RR --> AV[Published and Active Agent Version]
...
| MAIN-4 | 正式候选 | Release Operator 提交 exact Draft revision | 重验 Draft、Release、权限和部署级 Binding Profile | 形成 Formal Production Agent Candidate | 不接受 latest Draft、独立 manifest 或环境替代 Release | P1 | 目标已确认 |
| MAIN-5 | 发布激活 | Phase F 证据与在线 smoke 通过 | 冻结 Published Agent Version，并以 PostgreSQL CAS 激活 | Active Agent Version 更新 | 并发、审计、exact evidence、失败无状态改变 | P1 | 当前实现未贯通 Draft |
...
| LOOP-003 | 正式发布 | Formal Production Agent Candidate 必须根植于 exact Draft revision | Draft identity 与 revision | 可发布候选 | 不接受独立 manifest 或 mutable latest Draft | 已确认 |
...
| LOOP-007 | 外部连接 | KSS 拥有非敏感 Connection Profile 的版本化逻辑状态；Vault 拥有凭据值；部署拥有 connector、egress、trust root 和硬限制策略 | Profile Draft、Secret Handle、部署策略 | Published Profile revision | Worker 只解析通过部署策略准入的 exact revision；浏览器不接触 secret | 已确认 |
| LOOP-008 | Base 组合 | KSS 维护版本化、不可查询的 Base Draft；成员使用 `exact` 或 `latest_ready_at_preparation`，并在 Preparation 启动时一次性冻结 | exact Base Draft revision | immutable Knowledge Base Version 与 Preparation | Query 不读取 Draft/latest；Source 更新不自动发布或更新 Agent | 已确认 |
| LOOP-009 | Release 退役 | deprecated 阻止新采用但保护既有运行；retired 需要零可执行引用和 retention；revoked 仅用于安全或严重数据问题并立即失败关闭 | exact Release、reference facts、bounded reason、exact confirmation | lifecycle state、affected active Reference count 与审计 | 不自动替换 Release；物理删除是后续独立命令 | TDD-03B 至 03E 已实现 application-only deprecate、可信 Reference 注销准入、普通 retire 和 emergency revoke 核心；网络权限、明细投影/通知与 ProofAgent 失败关闭接线待实现 |
| LOOP-010 | 引用权威 | 每个客户端在外部资源可执行前向 KSS 幂等注册 exact Release reference；只有 owning client 及服务端可信 verifier 证明 external resource 永久失去执行与回滚资格后才可注销；KSS 账本是普通退役的本地权威 | client/resource/Release exact identity、trace-safe verification identity | durable active/deregistered Reference 或稳定拒绝 | 注册失败不激活；不确定或验证失败保留安全孤儿引用；后台 reconciler 不可按 TTL 盲删；emergency revoke 保留 Reference facts | TDD-03A 至 03E 已实现注册、可信注销准入、零 active Reference 退役门及与紧急撤销的竞争收敛；ProofAgent verifier 和后台 reconciler 待实现 |
...
| 交易与数据一致性 | 是 | Source/Release 不可变；Preparation 启动冻结 exact Version plan；TDD-02E 在一个 KSS PostgreSQL 事务提交 Release 与 consumed/expired；TDD-02F 原子提交 one-shot expired 与审计；TDD-02G 原子提交 queued/running → cancelled 与幂等收据/审计并 fence 旧 Worker | publish/expiry/cancel HTTP 权限、自动调度、既有直接入口收敛、正式 Agent 激活 CAS | P1 |
| 状态流转 | 是 | TDD-03A 至 03E 已持久化 `active → deregistered` `published_agent_version` 引用，并原子实现 `queryable → deprecated → retired` 普通路径和 `queryable/deprecated → revoked` 紧急路径；retired/revoked 从 Catalog、完整性扫描和既有 Query authorization 中移除 | ProofAgent verifier、后台 reconciler、受影响运行/回滚接线与重试运行责任 | P1 |
| 幂等与并发 | 是 | 长命令、Draft 保存和激活需要幂等/CAS | 管理命令的统一幂等保留策略 | P1 |
| 权限与审计 | 是 | 三个粗粒度角色包映射到全局 named permissions，可叠加；服务端授权与审计 | 角色映射和拒绝路径的实现验证 | P1 |
...
| --- | --- | --- | --- | --- |
| 当前无未决 P1 架构问题 | P1 | 已确认权威、Base composition、角色、Release lifecycle、Reference Ledger、Preparation 状态与正式发布候选；实现事实仍未达到目标设计 | KSS、ProofAgent、Dashboard、测试与发布负责人 | 按 Scope TDD 切片实施；完成真实依赖和 Phase F 前不得称 Production GO |
