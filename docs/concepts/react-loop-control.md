...
The bundled local file adapters are intentionally development/test and POSIX-only: construction requires a private application-owned root and fails closed unless anchored `dir_fd`, `O_NOFOLLOW`, and hard-link publication are available. Descriptor anchoring detects path replacement but does not isolate an arbitrary process with the same effective user. Production, sandbox execution, and any same-UID untrusted workload must use an isolated S3-compatible artifact adapter under ADR-0109; there is no path-based compatibility fallback.
