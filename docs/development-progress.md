...
| Knowledge Hub provider set | Active code accepts `local_markdown`, `local_index`, and the fixture `remote_search` adapter; `pageindex` and `local_vector` are rejected; Enterprise QA and Controlled ReAct now enter retrieval through the shared Control Plane Knowledge Retrieval Service; blended single-step and reviewed/fallback retrieval uses deterministic binding metadata routing, records provider calls, applies binding failure modes, exact deduplication, WRRF ordering, and no-evidence reason codes | Extend planner/evaluator-backed agentic retrieval with service-routed provider calls, then add READY Local Index runtime load and trusted `http_json` remote adapter |
| Agentic RAG | Agentic retrieval strategy emits governed retrieval plan/step events through the shared Knowledge Retrieval Service and falls back through registered Knowledge Providers when planner/evaluator models are absent; Controlled ReAct submits reviewed Retrieval Intent without directly calling providers | Planner-driven multi-step retrieval with nested retrieval budgets and richer trace-safe plan summaries |
