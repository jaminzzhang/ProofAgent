...
| Knowledge Hub provider set | Active code accepts `local_markdown`, `local_index`, and the fixture `remote_search` adapter; `pageindex` and `local_vector` are rejected; Enterprise QA and Controlled ReAct now enter retrieval through the shared Control Plane Knowledge Retrieval Service; blended retrieval records provider calls, applies binding failure modes, exact deduplication, and WRRF ordering | Extend the service with deterministic source routing, READY Local Index runtime load, richer no-evidence mapping, and trusted `http_json` remote adapter |
