...
| Delivery | `delivery/cli.py` exposes Typer commands including continuous `knowledge-worker` and bounded `knowledge-worker --once`; `delivery/api.py` exposes Run Execution and Conversation APIs; `delivery/configuration_api.py` exposes Agent Configuration workflows, Shared Model Connection CRUD/lifecycle/reference/validation/smoke-test routes, plus Local Index quarantine, ingestion-job status, derived candidate validation, and frozen snapshot management; `delivery/published_agents.py` resolves static Published Agents and Active Agent Versions |
