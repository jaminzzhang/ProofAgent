...
- Workflow Stage Prompt Configuration now adds backend-owned Workflow Template
  Descriptors, governed `workflow.stages[]` Prompt/context config for
  `react_enterprise_qa`, redacted stage context preview, Dashboard relationship-map
...
| Control | `control/` owns Enterprise QA and Controlled ReAct Enterprise QA workflow, Workflow Template Descriptors, stage context preview/summary assembly, policy, review fail-closed behavior, validators, evidence decisions, approval, and outcome behavior |
| Runtime | `runtime/langgraph_runner.py` executes supported LangGraph `StateGraph` templates with composed Harness dependencies; Controlled ReAct applies configured stage Business Context Addenda without replacing Harness prompts |
