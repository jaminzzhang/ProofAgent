...
| Delivery | `delivery/cli.py` exposes Typer commands including continuous `knowledge-worker` and bounded `knowledge-worker --once`; `delivery/api.py` exposes Run Execution and Conversation APIs; `delivery/configuration_api.py` exposes Agent Configuration workflows, Workflow Template Descriptor and workflow node preview/update routes, Shared Model Connection CRUD/lifecycle/reference/validation/smoke-test routes, plus Local Index quarantine, ingestion-job status, derived candidate validation, and frozen snapshot management; `delivery/published_agents.py` resolves static Published Agents and Active Agent Versions |
...
| Dashboard UI | Implemented for overview, runs, run detail, evidence, receipt, model usage, approvals, timeline, governed ReAct details, handoffs, Configuration > Models, Shared Model Connection detail/reference/test/audit views, Knowledge Hub Source creation/detail/publication/lifecycle controls with source-owned model selectors, and the Agents configuration workspace with descriptor-driven Workflow relationship map/node Prompt editing, Model/Knowledge editing, active published Source binding, validation, publish, and rollback | Approval Console actions, RBAC, richer multi-agent operations, shared Prompt templates, model connection import/export, and a real secret vault |
