...
- Workflow Node Prompt Configuration now adds backend-owned Workflow Template
  Descriptors, governed `workflow.nodes[]` Prompt/context config for
  `react_enterprise_qa`, redacted node context preview, Dashboard relationship-map
  editing, validation/publish gating, and runtime Business Context Addendum injection
  with trace-safe `workflow_node_context_applied` events. Harness-owned prompts remain
  locked and node Prompt text is never stored in trace.
...
| Control | `control/` owns Enterprise QA and Controlled ReAct Enterprise QA workflow, Workflow Template Descriptors, node context preview/summary assembly, policy, review fail-closed behavior, validators, evidence decisions, approval, and outcome behavior |
| Runtime | `runtime/langgraph_runner.py` executes supported LangGraph `StateGraph` templates with composed Harness dependencies; Controlled ReAct applies configured node Business Context Addenda without replacing Harness prompts |
...
| --- | --- | --- |
| LangGraph runtime | Enterprise QA `StateGraph` runs through composed Harness dependencies; approval decisions use `PendingApproval` plus Approval Checkpoint Resume with local checkpoint persistence, restart recovery, per-run local resume claim, Local Operator Identity Provider actor capture, and expired-approval fail-closed behavior | Shared transactional checkpointer/lock backend for multi-instance deployments, production auth/RBAC integration, and streaming hooks |
| Controlled ReAct | `react_enterprise_qa` runs deterministic planner/review scenarios with fixed action set, advisory review, clarification wait, approval wait, trace, receipt output, and checkpoint-backed approval resume | Streaming and remote planner/review adapter hardening |
...
| Dashboard UI | Implemented for overview, runs, run detail, evidence, receipt, model usage, Run Detail approval actions against Approval Checkpoint Resume through server-side Operator Identity Context, global approval queue API projection and `/approvals` triage page, timeline, governed ReAct details, handoffs, Configuration > Models and Shared Model Connection detail/reference/test/audit views through API-resolved operator identity instead of frontend actor fields, Knowledge Hub Source creation/detail/publication/lifecycle controls with source-owned model selectors and API-resolved operator identity, and the Agents configuration workspace with descriptor-driven Workflow relationship map/node Prompt editing, Model/Knowledge editing, active published Source binding, validation, publish, and rollback through API-resolved operator identity | Production auth/RBAC integration, richer multi-agent operations, shared Prompt templates, model connection import/export, and a real secret vault |
