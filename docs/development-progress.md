...
| Dashboard UI | Implemented for overview, runs, run detail, evidence, receipt, model usage, approvals, timeline, governed ReAct details, handoffs, Knowledge Hub Source creation/detail/publication/lifecycle controls, and the Agents configuration workspace with Workflow node editing, active published Source binding, validation, publish, and rollback | Approval Console actions, RBAC, and richer multi-agent operations |
